    ...
    <div class="content-ads" v-if="ads.data && ads.data.items && ads.data.items.length>0">
      <div class="img-pannel" v-if="ads.data.type == '1'">
        <div v-if="ads.data.items.length == 1">
          <a :href="ads.data.items[0].link" target="_blank"><img :src="ads.data.items[0].sImg"
                                                                 :alt="ads.data.items[0].alt"/></a>
        </div>
        <div v-else>
          <!-- 轮播展示 -->
          <div v-if="ads.data.carousel">
            <el-carousel :height="ads.data.height+'px'">
              <el-carousel-item v-for="item in ads.data.items" :key="item._id">
                <h3>
                  <a :href="item.link" target="_blank"><img :height="ads.data.height+'px'" :src="item.sImg"
                                                            :alt="item.alt"/></a>
                </h3>
              </el-carousel-item>
            </el-carousel>
          ...
          <div v-else>
            <el-row :gutter="20">
              <el-col class="case-item" :xs="12" :sm="8" :md="6" :lg="6" :xl="6" v-for="(item,index) in ads.data.items"
                      :key="item._id">
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      console.log(111, adsList.data, this.id)
